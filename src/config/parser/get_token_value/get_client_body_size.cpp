		...
		char const *config_file, int &error_count)
{
	...
	if (i == tokens.size())
	{
		std::cout << std::string(config_file) + ": error, expected a semicolon, to end the client_body_size_limit"
			" attribute at " + position.to_string() + ", but found end of file\n";
		error_count++;
		return  ;
	}
	if (tokens[i].get_token()[0] != ';')
	{
		std::cout << std::string(config_file) + tokens[i].get_position().to_string() +
		   	": error, expected a semicolon, to end the client_body_size_limit attribute at " + position.to_string() +
