...
ReturnType handle_error(int error_code, t_c_individual_server_config const &config)
{
	...
	switch(error_code)
	{
		...
		{
			head << "Bad Request\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_bad_request())->_fileno;
			break;
		}
		case 403:
		{
			head << "Forbiden\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_forbidden())->_fileno;
			break;
		}
		case 404:
		{
			head << "Not found\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_not_found())->_fileno;
			break;
		}
		case 408:
		{
			head << "Request timeout\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_request_timeout())->_fileno;
			break;
		}
		case 411:
		{
			head << "Length required\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_length_requiered())->_fileno;
			break;
		}
		case 413:
		{
			head << "Content too large\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_content_too_large())->_fileno;
			break;
		}
		case 414:
		{
			head << "Uri too long\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_uri_too_long())->_fileno;
			break;
		}
		case 500:
		{
			head << "Internal server error\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_internal_server_error())->_fileno;
			break;
		}
		case 501:
		{
			head << "Not implemented\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_not_implemeted())->_fileno;
			break;
		}
		case 505:
		{
			head << "HTTP version not supported\r\n\r\n";
			fd = write_response(head.str(), error_pages->get_http_version_not_supported())->_fileno;
			break;
		}
	}
	return(ReturnType(fd, 0));
}
